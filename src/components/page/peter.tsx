...
const Peter: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadingTask = pdfjsLib.getDocument("/comic/peter.pdf");
  ...
  const renderAllPages = async (
    ...
    if (containerRef.current) {
      // Clear previous canvases to prevent duplicates
  ...
  const handlePDFDownload = () => {
    ...
    link.href = "/comic/peter.pdf";
    link.download = "peter.pdf";
    link.click();
  };
  ...
  return (
    <div className="bg-crimson w-screen h-screen flex items-center justify-center relative overflow-auto">
      ...
      <div className="absolute right-11 top-10 bg-peter justify-center text-center shadow-md shadow-white rounded w-20 cursor-pointer hover:scale-110">
        <a href="/add" className="font-inspiration">
          Add on's
        </a>
      </div>
    </div>
